...
const WashedIroned = () => {
  ...
  // Fetch backend data for WASHED_IRONED services
  const { data: laundryServicesData, isLoading } = useGetLaundryServices({
    laundry_type: 'WASHED_IRONED'
  });
  ...
  useEffect(() => {
    if (laundryServicesData?.data && laundryServicesData.data.length > 0) {
      ...
      const mappedServices = getServicesForType(laundryServicesData.data, 'WASHED_IRONED');
      ...
      setExtraServices(mappedServices.extra.length > 0 ? mappedServices.extra : extraLaundryServices);
    } else {
